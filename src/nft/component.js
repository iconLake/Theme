...
import { scss, html } from '../api/index.js?nft'

export default class Nft extends HTMLElement {
  constructor () {
    super()
    ...
    window.iconlakeAPI.nft.getInfo(window.iconlakeAPI.nft.id).then((nft) => {
      this.renderNft(nft)
  ...
  renderNft (nft) {
    const dom = html(`
      <div class="nft">
        ...
        <div class="info">
          ...
          <div class="item">
            <div>创作者：</div>
            <div>${nft.data.author}</div>
          </div>
          <div class="item">
            <div>时　间：</div>
            <div>${new Date(+nft.data.createTime).toLocaleString()}</div>
          </div>
          <div class="item">
            <div>系　列：</div>
            <a href="/exhibition/${nft.classId}">${nft.classId}&nbsp;
            <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="9" height="9" fill="currentColor"><path d="M644.87517299 510.17438549l-367.65643436 355.41154361c-21.67800946 20.95381845-21.67800946 54.92445525 0 75.84450789 21.67917341 20.95381845 56.78732743 20.95381845 78.46650084 0L762.59355307 548.07801059c21.67800946-20.95381845 21.67800946-54.92445525 0-75.84450787l-406.90714966-393.31633267c-10.83842219-10.47632669-25.04515129-15.69702571-39.25071531-15.6970257-14.20556402 0-28.41229198 5.22069902-39.21578552 15.73428338-21.67800946 20.95381845-21.67800946 54.88952547 0 75.84450788L644.87517299 510.17438549z"></path></svg>
            </a>
          </div>
          <div class="item">
            <div>编　号：</div>
    ...
    window.iconlakeAPI.verifyHash(nft.uri, nft.uriHash).then((res) => {
      ...
      const verifyResult = result[res.toString()] || result['-1']
      resDom.classList.add(verifyResult.className)
      resDom.setAttribute('title', verifyResult.title)
      resDom.innerHTML = verifyResult.icon
    })
  }
}
